...
struct keystone_pcie {
	...
	/* PCI Device ID */
	u32			device_id;
	struct			device_node *intx_intc_np;
...
static void ks_pcie_am654_intx_irq_handler(struct irq_desc *desc)
{
	...
	for (i = 0; i < PCI_NUM_INTX; i++) {
		...
		virq = irq_linear_revmap(ks_pcie->intx_irq_domain, i);
...
static int ks_pcie_intx_irq_domain_alloc(struct irq_domain *domain, unsigned int virq,
					   unsigned int nr_irqs, void *data)
{
	struct keystone_pcie *ks_pcie = domain->host_data;
	struct device_node *np = ks_pcie->intx_intc_np;
...
static const struct irq_domain_ops ks_pcie_intx_irq_domain_ops = {
	.alloc		= ks_pcie_intx_irq_domain_alloc,
...
static int ks_pcie_am654_config_intx_irq(struct keystone_pcie *ks_pcie)
{
	struct device *dev = ks_pcie->pci->dev;
	struct irq_domain *intx_irq_domain;
	struct device_node *np = ks_pcie->np;
	...
	irq_set_chained_handler_and_data(irq, ks_pcie_am654_intx_irq_handler, ks_pcie);
...
static int ks_pcie_config_intx_irq(struct keystone_pcie *ks_pcie)
{
	...
	intx_irq_domain = irq_domain_add_hierarchy(parent_domain, 0, PCI_NUM_INTX, intc_np,
						     &ks_pcie_intx_irq_domain_ops, ks_pcie);
	if (!intx_irq_domain) {
...
static int __init ks_pcie_host_init(struct dw_pcie_rp *pp)
{
	...
	if (!ks_pcie->is_am6) {
		...
		ret = ks_pcie_am654_config_intx_irq(ks_pcie);
		if (ret)
			return ret;
	}
